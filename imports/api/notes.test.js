...
if (Meteor.isServer) {
  describe('notes', function () {
    ...
    it('should not remove note in invalid _id', function () {
      expect(() => {
    ...
    it('should throw error if extra updates', function () {
      expect(() => {
        Meteor.server.method_handlers['notes.update'].apply({
          ...
          noteOne._id,
          { title: 'New Title', name: 'Joe' }
        ]);
      }).toThrow();
    })

    it('should note update not if user was not creator', function () {
      const title = 'this is an updated title';
      Meteor.server.method_handlers['notes.update'].apply({
        userId: 'testid'
      },[
    ...
    it('should not update note in invalid _id', function () {
      expect(() => {
    ...
    it('should return zero notes for user that has none', function () {
      const res = Meteor.server.publish_handlers.notes.apply( {userId: 'bubba' });
      const notes = res.fetch();
